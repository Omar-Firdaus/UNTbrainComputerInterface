...
@dataclass
class EmotivStream:
    """Real-time EEG from Emotiv via Cortex API."""
    ...
    def from_env(cls, **kwargs) -> "EmotivStream":
        ...
        if not client_id or not client_secret:
            raise EnvironmentError("EMOTIV_CLIENT_ID and EMOTIV_CLIENT_SECRET must be set in .env")
    ...
    def connect(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run_loop, daemon=True, name="emotiv-cortex")
        self._thread.start()

        # wait for auth + headset subscription to complete
        deadline = time.monotonic() + 30.0
        while self._headset is None and time.monotonic() < deadline:
            if not self._running:
                raise RuntimeError("Emotiv stream failed to start. Check EMOTIV App and credentials.")
            time.sleep(0.1)
        if self._headset is None:
            raise TimeoutError("Timed out waiting for Emotiv headset. Is the EMOTIV App open?")
    ...
    async def _cortex_session(self) -> None:
        ...
        # Cortex uses a self-signed cert
    ...
    async def _connect_headset(self, ws) -> HeadsetInfo:
        ...
        if not headsets:
            raise RuntimeError("No Emotiv headsets found. Pair your headset in the EMOTIV App.")

        if self.headset_id:
            matched = [h for h in headsets if h["id"] == self.headset_id]
            if not matched:
                raise RuntimeError(f"Headset '{self.headset_id}' not found. Available: {[h['id'] for h in headsets]}")
            h = matched[0]
        else:
            h = headsets[0]

        if h.get("status") != "connected":
            await self._send(ws, "controlDevice", {"command": "connect", "headsetId": h["id"]}, req_id=4)
            await asyncio.sleep(2.0)

        return HeadsetInfo(headset_id=h["id"], status=h.get("status", "unknown"), n_channels=0, fs=0.0, channel_names=[])
    ...
    async def _subscribe_eeg(self, ws) -> tuple[list[str], float]:
        ...
        channel_names = [c for c in eeg_info.get("cols", []) if c.upper() not in skip]
    ...
    async def _read_eeg(self, ws) -> None:
        while self._running:
            ...
            if "eeg" not in msg:
                continue
            # eeg row: [counter, interpolated, ch0..chN, marker]
            samples = msg["eeg"][2: 2 + self._headset.n_channels]
            with self._lock:
...
def stream_csa(
    ...
    """Yield a CSAResult each time a new epoch is ready from a live EmotivStream."""
    fs = stream.fs
    ...
    while True:
        ...
        result = compute_csa(signal, fs=fs, epoch_sec=epoch_sec, overlap=0.0, fmax=fmax)
        yield result
        last_yield_at = total
        time.sleep(poll_interval)
...
@dataclass
class PowStream:
    """Real-time band-power stream via Emotiv Cortex 'pow' subscription.

    Streams theta/alpha/betaL/betaH/gamma per channel.
    Does not require an Advanced Emotiv license.
    """
    ...
    def from_env(cls, **kwargs) -> "PowStream":
        ...
        client_secret = os.environ.get("EMOTIV_CLIENT_SECRET", "")
        if not client_id or not client_secret:
            raise EnvironmentError("EMOTIV_CLIENT_ID and EMOTIV_CLIENT_SECRET must be set.")
        return cls(client_id=client_id, client_secret=client_secret, **kwargs)
    ...
    def connect(self) -> None:
        ...
        self._loop = asyncio.new_event_loop()
        self._thread = threading.Thread(target=self._run_loop, daemon=True, name="emotiv-pow")
        self._thread.start()
    ...
    async def _cortex_session(self) -> None:
        ...
        async with websockets.connect(_CORTEX_URL, ssl=ssl_ctx) as ws:
            await self._send(ws, "requestAccess", {"clientId": self.client_id, "clientSecret": self.client_secret}, req_id=1)
            result = await self._send(ws, "authorize", {"clientId": self.client_id, "clientSecret": self.client_secret, "debit": 1}, req_id=2)
            ...
            h = next((x for x in headsets if x["id"] == self.headset_id), headsets[0]) if self.headset_id else headsets[0]

            sess = await self._send(ws, "createSession", {
                "cortexToken": self._token, "headset": h["id"], "status": "active"
            }, req_id=4)
            self._session_id = sess["id"]

            sub = await self._send(ws, "subscribe", {
                "cortexToken": self._token, "session": self._session_id, "streams": ["pow"]
            }, req_id=5)
            ...
            # parse "CH/band" column names into separate lists
            ch_names, band_names = [], []
            for col in success[0].get("cols", []):
                ch, band = col.split("/")
...
def stream_pow(stream: PowStream, poll_interval: float = 0.05) -> Iterator[PowFrame]:
    """Yield a PowFrame for each new band-power sample from a live PowStream."""
    last_yield_at = 0
